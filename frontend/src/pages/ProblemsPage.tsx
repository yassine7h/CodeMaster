  ...
  return (
    <tr key={problem.id} className="hover:bg-gray-700">
      <td className="py-2 px-4 font-semibold">
        <Link to={problemLink} className="hover:text-yellow-400">{problem.id}</Link>
      </td>
      <td className="py-2 px-4">
        <Link to={problemLink} className="hover:text-yellow-400">{problem.title}</Link>
      </td>
      <td className="py-2 px-4">
        <Link to={solutionLink} className="hover:text-yellow-400">See solution</Link>
      </td>
      <td className="py-2 px-4">{problem.acceptance}%</td>
      <td className={`py-2 px-4 font-semibold ${difficultyClass[problem.difficulty]}`}>
        {problem.difficulty}
      </td>
    </tr>
  );
...
export default function ProblemsPage() {
  ...
  const filteredProblems = problems.filter((problem) => {
    ...
    const matchAcceptance =
      filterAcceptanceRange === "All" ||
      (problem.acceptance >= minAcceptance && problem.acceptance <= maxAcceptance);
